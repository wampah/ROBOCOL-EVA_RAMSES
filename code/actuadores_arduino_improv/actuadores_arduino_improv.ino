...
void setup() {
  ...
  servomotores(servo_caja1, 0);
  servomotores(servo_caja2, 0);
  servomotores(servo_caja3, 0);
  servomotores(servo_caja4, 0);

  servomotores(servo_disp, degs_reposo);
  servomotores(servo_niv, 70);
  servomotores(servo_ex, degs_reposo);
...
void actuador_manual_RAMSES(boolean dir, double distancia_grados) {
  if (dir) {
    stepper_RAMSES.move((int)(MICROSTEPS_RAMSES * factor_RAMSES * distancia_grados));
    while (stepper_RAMSES.distanceToGo() != 0) {
      stepper_RAMSES.run();
      Serial.println(((double)stepper_RAMSES.currentPosition()) / (MICROSTEPS_RAMSES * factor_RAMSES ));
    }
  }
  else {
    stepper_RAMSES.move((int)(-MICROSTEPS_RAMSES * factor_RAMSES * distancia_grados));
...
void actuador_auto_RAMSES(boolean pos) {
  int angulo_temp;
  int counter = 0;
  if (pos) {
    stepper_RAMSES.moveTo(0 * MICROSTEPS_RAMSES * factor_RAMSES );
    while (stepper_RAMSES.distanceToGo() != 0) {

      stepper_RAMSES.run();
      if (counter == 500) {
        angulo_temp = (int)(70 + ((double)stepper_RAMSES.currentPosition()) / (MICROSTEPS_RAMSES * factor_RAMSES*1.6) );

        servomotores(servo_niv, abs(angulo_temp));

         counter = 0;
      }
      counter++;
      Serial.println(angulo_temp);


    }
  }
  else {
    stepper_RAMSES.moveTo(-120 * MICROSTEPS_RAMSES * factor_RAMSES );
    while (stepper_RAMSES.distanceToGo() != 0) {
      stepper_RAMSES.run();
      if (counter == 500) {
        angulo_temp = (int)(70 + ((double)stepper_RAMSES.currentPosition()) / (MICROSTEPS_RAMSES * factor_RAMSES*1.6) );
...
void caja_EVA(int caja, boolean estado) {
  if (estado == true) {
    if (caja == 1) {
      servomotores(servo_caja1, 180);
    }
    else if (caja == 2) {
      servomotores(servo_caja2, 180);
    }
    else if (caja == 3) {
      servomotores(servo_caja3, 180);
    }
    else if (caja == 4) {
      servomotores(servo_caja4, 180);
    }
    else {}
  }
...
void dispensar_manual_EVA(int vel, int t_segundos) {
  servomotores(servo_disp, (int)(90.0 - (90.0 * ( (double)vel / 5))));
  delay(t_segundos * 1000);
  servomotores(servo_disp, degs_reposo);
  delay(1000);
}
void extraer_RAMSES(int vel, int t_segundos) {
  digitalWrite(PIN_TAMIZ, LOW);
  servomotores(servo_ex, (int)(90.0 - (90.0 * ( (double)vel / 5))));
  delay(t_segundos * 1000);
  servomotores(servo_ex, degs_reposo);
  delay(3000);
  digitalWrite(PIN_TAMIZ, HIGH);
...
void serialEvent() {
  while (Serial.available()) {
    ...
    if (comando == 'a' || comando == 'A') {
      int pos_temp = String(inputString.charAt(1)).toInt();
      if (pos_temp == 1) {
        Serial.println("Moving to position 0°");
        actuador_auto_RAMSES(pos_temp);
        Serial.println("Moved to position 0°");
      }
      else if (pos_temp == 0) {
        Serial.println("Moving to position 120°");
        actuador_auto_RAMSES(pos_temp);
        Serial.println("Moved to position 120°");
      }
      else {
        Serial.println(error_msg + "Invalid compartment");
      }
    ...
    if (comando == 'x' || comando == 'X') {
      int vel_temp = String(inputString.charAt(1)).toInt();
      int t_disp_temp = String(inputString.charAt(2)).toInt() * 10 + String(inputString.charAt(3)).toInt();
      if (vel_temp > 0 && vel_temp <= 5) {
        if (t_disp_temp > 0 && t_disp_temp <= 90) {
          Serial.println("Extracting sample at speed " + String(vel_temp) + " for " + String(t_disp_temp) + " seconds");
          extraer_RAMSES(vel_temp, t_disp_temp);
          Serial.println("Sample extracted");
        }
        else {
          Serial.println(error_msg + "Invalid dispensing time");
        }
    ...
    if (comando == 'l' || comando == 'L') {
      int angle_temp = String(inputString.charAt(1)).toInt() * 100 + String(inputString.charAt(2)).toInt() * 10 + String(inputString.charAt(3)).toInt();
      if (angle_temp >= 0 && angle_temp <= 180) {
        servomotores(servo_niv, angle_temp);
      }
      else {
        Serial.println("F");
      }
...
void servomotores(int servo, int grados) {
